import { Star, Timer, Network, Grid3X3 } from "lucide-react";
...
export function ActionButtons({ layoutMode, onButtonClick, onLayoutToggle }: ActionButtonsProps) {
  const buttons = [
    ...
    { icon: Timer, action: "timer" },
    { icon: Network, action: "network" },
    { icon: Grid3X3, action: "grid" },
  ];
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <div className="relative flex justify-center items-end space-x-4">
          ...
          <div className="relative top-5">
            ...
            <div className="absolute -top-8 -right-8 transform -translate-x-1/2">
              <div className="relative">
                
                <button
                  onClick={onLayoutToggle}
                  className="bg-gradient-to-b from-amber-400 via-amber-500 to-amber-600 text-white px-4 py-2 rounded-lg shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 border border-amber-300"
                >
                  {/* Inner highlight */}
                  <div className="absolute inset-0 bg-gradient-to-b from-white/20 via-white/5 to-transparent rounded-lg" />
                  
                  <span className="text-xs font-bold drop-shadow-sm relative"></span>
    ...
    <div className="absolute bottom-0 left-0 right-0 p-6 space-y-4">
      ...
      <div className="flex justify-center">
        <div className="relative">
          ...
          <button
            onClick={() => onButtonClick?.("create-bees")}
            className="relative scale-[1.2] bg-gradient-to-b from-amber-400 via-amber-500 to-amber-600 text-white py-3 px-16 rounded-2xl shadow-2xl border-2 border-amber-300 hover:scale-125 active:scale-95 transition-all duration-200"
          >
